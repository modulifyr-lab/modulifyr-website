...
import { CheckCircle2, AlertCircle, Loader2, ArrowRight, ShieldCheck, Globe, MapPin } from "lucide-react";

const industries = ["Education", "Commerce", "Healthcare", "IT", "Retail", "Services", "Other"];
const projectTypes = [
    "Custom Software Development", "System Architecture & Consulting", "Integrations & Automation",
    "Modernization & Refactoring", "Infrastructure & SRE", "Data Architecture & Analytics", "Other / Not Sure Yet",
];
const budgetsNepal = [
    "NPR 200000 – 450000 (Discovery Phase)", "NPR 600000 – 1500000 (Pilot / Proof-of-Value)",
    "NPR 1500000 – 3000000 (Focused Build)", "NPR 3000000 – 6000000 (Full System)",
    "NPR 6000000+ (Enterprise / Team)", "Not Sure Yet",
];
const budgetsInternational = [
    "$6000 – $12000 (Discovery Phase)", "$15000 – $35000 (Pilot / Proof-of-Value)",
    "$35000 – $70000 (Focused Build)", "$70000 – $150000 (Full System)",
    "$150k+ (Enterprise / Team)", "Not Sure Yet",
];
...
const initialForm: FormState = {
    ...
    industry: "", project_type: "", budget: "", message: "", website: "",
};
                    ...
                    <div className="container-custom">
                        <div className="max-w-2xl mx-auto text-center flex flex-col items-center gap-8">
                            ...
                            <div className="flex flex-wrap gap-4 justify-center mt-4">
                                ...
                                <Button><a href="/process">View Our Process</a></Button>
                            </div>
                        </div>
                    </div>
                </section>
            ...
            <section className="bg-brand-navy text-white py-20 border-b border-white/10">
                <div className="container-custom">
                    <div className="max-w-4xl">
                        <span className="text-brand-orange font-bold tracking-widest uppercase text-xs mb-4 block">Request for Proposal</span>
                        <h1 className="text-4xl md:text-6xl font-heading font-bold mb-6">Tell Us About Your <span className="text-brand-orange">System</span></h1>
                        <p className="text-xl text-text-muted leading-relaxed max-w-2xl">Submit your project requirements and receive a structured technical proposal, architecture plan, and cost estimate within 48 hours.</p>
            ...
            <div className="bg-brand-navy/5 border-b border-border-base py-3">
                <div className="container-custom flex flex-col sm:flex-row items-start sm:items-center gap-3">
                    ...
                    <div className="flex gap-2">
                        <button type="button" onClick={() => setRegion("nepal")}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
                        <div className="lg:col-span-2">
                            <div className="bg-white border border-border-base rounded-3xl p-8 md:p-12 shadow-sm">
                                ...
                                {/* Section 01 */}
                                <div className="mb-10">
                                    <h2 className="text-xs font-bold text-text-muted uppercase tracking-widest mb-6 pb-3 border-b border-border-base">01 — Contact Information</h2>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        {[
                                            ...
                                            <div key={f.name} className="flex flex-col gap-2">
                                                ...
                                                <input type={f.type} name={f.name} value={(form as Record<string, string>)[f.name]} onChange={handleChange} placeholder={f.placeholder}
                                ...
                                {/* Section 02 */}
                                <div className="mb-10">
                                    <h2 className="text-xs font-bold text-text-muted uppercase tracking-widest mb-6 pb-3 border-b border-border-base">02 — Project Details</h2>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        ...
                                        <div className="flex flex-col gap-2 md:col-span-2">
                                            <div className="flex items-center justify-between">
                                                <label className="text-sm font-semibold text-brand-navy">Budget Range ({isNepal ? "NPR" : "USD"}) <span className="text-brand-orange">*</span></label>
                                                <button type="button" onClick={() => setRegion(isNepal ? "international" : "nepal")} className="text-xs text-brand-orange hover:underline font-medium">Switch to {isNepal ? "USD" : "NPR"}</button>
                                            </div>
                                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                                                {budgets.map(b => (
                                                    <button key={b} type="button" onClick={() => setForm(prev => ({ ...prev, budget: b }))}
                                                        className={`px-3 py-3 rounded-xl border text-xs font-semibold text-left transition-all cursor-pointer ${form.budget === b ? "border-brand-orange bg-brand-orange/5 text-brand-orange" : "border-border-base bg-bg-light text-text-secondary hover:border-brand-navy"}`}>
                                                        {b}
                                                    </button>
                                                ))}
                                            </div>
                                ...
                                {/* Section 03 */}
                                <div className="mb-10">
                                    <h2 className="text-xs font-bold text-text-muted uppercase tracking-widest mb-6 pb-3 border-b border-border-base">03 — Project Brief</h2>
                                    <div className="flex flex-col gap-2">
                                ...
                                <Button size="lg" onClick={handleSubmit} disabled={status === "loading"} className="w-full justify-center group">
                                    {status === "loading" ? <><Loader2 className="w-5 h-5 mr-2 animate-spin" /> Submitting...</> : <>Submit Proposal Request <ArrowRight className="w-5 h-5 ml-2 group-hover:translate-x-1 transition-transform" /></>}
                                </Button>
                                <p className="text-xs text-text-muted text-center mt-4">
                                    By submitting, you agree to our <a href="/privacy" className="text-brand-teal hover:underline">Privacy Policy</a>. We respond within 24 hours.
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-6">
                                ...
                                <div className="space-y-6">
                                    {[
                                        ...
                                        <div key={item.step} className="flex gap-4">
                                            ...
                                            <div><p className="font-bold text-sm mb-1">{item.title}</p><p className="text-text-muted text-xs leading-relaxed">{item.desc}</p></div>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            <div className="bg-bg-secondary border border-border-base rounded-3xl p-8">
                                ...
                                <p className="text-sm text-text-secondary leading-relaxed mb-6">Book a free 15-minute discovery call directly with our lead architect.</p>
                                <a href="/contact"><Button variant="outline" className="w-full">Book Discovery Call</Button></a>
                            </div>
                            <div className="flex items-start gap-3 p-5 bg-brand-teal/5 border border-brand-teal/20 rounded-2xl">
                                ...
                                <p className="text-xs text-text-secondary leading-relaxed"><strong className="text-brand-navy">NDA Available.</strong> All information shared is treated confidentially. We can sign an NDA before any technical discussion.</p>
                            </div>
                        </div>
                    </div>
                </div>
